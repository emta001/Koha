...
sub push_loans {
    my $self = shift;
    my ($mongocol, $logs, $limit, $from_date, $to_date) = @_;
    my $loans = $self->{loans};
    my $items = $self->{items};
    ...
    try {
        ...
        my $check_collection = $generals->checkCollection($mongocol, $logs, $from_date, $to_date);
        if($check_collection){
            return;
        }
        ...
        if($return->acknowledged){
            $generals->writeLog($logs, $to_date, $mongocol->{name}, "OK");
        }

    } catch {

        $generals->writeLog($logs, $to_date, $mongocol->{name}, $_);
        warn "error: $_";
